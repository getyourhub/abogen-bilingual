...
class SubtitleWriter:
    def __init__(self, path: Path, format_key: str, *, bilingual_mode: Optional[str] = None, translation_config: Optional[Any] = None) -> None:
        self.path = path
        self.format_key = format_key
        self.bilingual_mode = bilingual_mode  # "original_on_top", "translation_on_top", "translation_only", or None
        self.translation_config = translation_config
        self._segments: List[Dict[str, Any]] = []
        self._translator = None
        self._translation_done = False
        
        if bilingual_mode and translation_config:
            try:
                from abogen.subtitle_translator import TranslationConfig, translate_subtitles
                self._translator = {
                    "config": translation_config,
                    "translate_func": translate_subtitles,
                }
            except ImportError:
                pass
    ...
    def close(self) -> None:
        ...
        # Perform translation if bilingual mode is enabled
        if self.bilingual_mode and self._translator and not self._translation_done:
            self._perform_translation()
        
        # Write the output file
        with self.path.open("w", encoding="utf-8", errors="replace") as f:
            if self.bilingual_mode:
                content = self._format_bilingual()
            else:
                content = self._format_original()
            f.write(content)
    
    def _perform_translation(self) -> None:
        """Translate all segments using the configured LLM."""
        try:
            from abogen.subtitle_translator import SubtitleEntry, TranslationConfig, translate_subtitles
            
            config = self._translator["config"]
            entries = []
            ...
            translated_entries = translate_subtitles(entries, config)
            
            # Update segments with translations
            for i, entry in enumerate(translated_entries):
                if i < len(self._segments):
                    self._segments[i]["translated_text"] = entry.translated_text
            
            self._translation_done = True
        except Exception as e:
            # If translation fails, continue without translation
            import sys
            print(f"Translation failed: {e}", file=sys.stderr)
            self._translation_done = True
    
    def _format_original(self) -> str:
        """Format subtitles without translation."""
        if self.format_key == "ass":
            return self._format_ass_original()
        return self._format_srt_original()
    
    def _format_srt_original(self) -> str:
        """Format as standard SRT."""
        lines = []
        for seg in self._segments:
    ...
    def _format_ass_original(self) -> str:
        """Format as standard ASS."""
        lines = []
        lines.extend(self._get_ass_header())
        for seg in self._segments:
            lines.append(
                f"Dialogue: 0,{_format_timestamp(seg['start'], ass=True)},{_format_timestamp(seg['end'], ass=True)},Default,,0000,0000,0000,,{seg['text']}"
            )
        return '\n'.join(lines)
    
    def _format_bilingual(self) -> str:
        """Format as bilingual subtitles."""
        if self.format_key == "ass":
            return self._format_ass_bilingual()
        return self._format_srt_bilingual()
    
    def _format_srt_bilingual(self) -> str:
        """Format as bilingual SRT."""
        lines = []
        for seg in self._segments:
            lines.append(str(seg["index"]))
            lines.append(f"{_format_timestamp(seg['start'])} --> {_format_timestamp(seg['end'])}")
            ...
            if self.bilingual_mode == "translation_only":
                lines.append(translated or seg["text"])
            elif self.bilingual_mode == "translation_on_top":
                if translated:
    ...
    def _format_ass_bilingual(self) -> str:
        """Format as bilingual ASS."""
        lines = []
        lines.extend(self._get_ass_bilingual_header())
        
        for seg in self._segments:
            start_ass = _format_timestamp(seg['start'], ass=True)
            end_ass = _format_timestamp(seg['end'], ass=True)
            translated = seg.get("translated_text")
            
            if self.bilingual_mode == "translation_only":
                if translated:
                    lines.append(f"Dialogue: 0,{start_ass},{end_ass},Translated,,0000,0000,0000,,{translated}")
                else:
                    lines.append(f"Dialogue: 0,{start_ass},{end_ass},Original,,0000,0000,0000,,{seg['text']}")
            elif self.bilingual_mode == "translation_on_top":
                if translated:
                    lines.append(f"Dialogue: 0,{start_ass},{end_ass},Translated,,0000,0000,0000,,{translated}")
                lines.append(f"Dialogue: 1,{start_ass},{end_ass},Original,,0000,0000,0000,,{seg['text']}")
            else:  # original_on_top
                lines.append(f"Dialogue: 1,{start_ass},{end_ass},Original,,0000,0000,0000,,{seg['text']}")
                if translated:
                    lines.append(f"Dialogue: 0,{start_ass},{end_ass},Translated,,0000,0000,0000,,{translated}")
    ...
    def _get_ass_bilingual_header(self) -> List[str]:
        """Get bilingual ASS header with two styles."""
        return [
            "[Script Info]",
            "Title: Generated by Abogen (Bilingual)",
            "ScriptType: v4.00+",
            "",
            "[V4+ Styles]",
            "Format: Name, Fontname, Fontsize, PrimaryColour, SecondaryColour, OutlineColour, BackColour, Bold, Italic, Underline, StrikeOut, ScaleX, ScaleY, Spacing, Angle, BorderStyle, Outline, Shadow, Alignment, MarginL, MarginR, MarginV, Encoding",
            "Style: Original,Arial,24,&H00FFFFFF,&H00808080,&H00000000,&H00404040,0,0,0,0,100,100,0,0,3,2,0,5,10,10,30,1",
            "Style: Translated,Arial,20,&H0000FFFF,&H00808080,&H00000000,&H00404040,0,0,0,0,100,100,0,0,3,2,0,5,10,10,10,1",
            "",
            "[Events]",
            "Format: Layer, Start, End, Style, Name, MarginL, MarginR, MarginV, Effect, Text",
        ]
...
def _create_subtitle_writer(job: Job, audio_path: Path) -> Optional[SubtitleWriter]:
    ...
    # Get bilingual subtitle settings
    bilingual_mode = getattr(job, "bilingual_subtitle_mode", None)
    translation_config = None
    
    if bilingual_mode:
        # Build translation config from job settings
        from abogen.subtitle_translator import TranslationConfig
        translation_config = TranslationConfig(
            base_url=getattr(job, "translation_base_url", "") or "",
            api_key=getattr(job, "translation_api_key", "") or "",
            model=getattr(job, "translation_model", "") or "",
            source_language=getattr(job, "translation_source_lang", "en"),
            target_language=getattr(job, "translation_target_lang", "zh"),
            timeout=float(getattr(job, "translation_timeout", 60.0)),
            batch_size=int(getattr(job, "translation_batch_size", 10)),
        )
        if translation_config.is_configured():
            job.add_log(f"Bilingual subtitles enabled: {bilingual_mode}", level="info")
        else:
            job.add_log("Bilingual subtitles requested but translation not configured. Disabling.", level="warning")
            bilingual_mode = None
            translation_config = None

    if fmt == "srt":
        return SubtitleWriter(
            audio_path.with_suffix(".srt"), "srt",
            bilingual_mode=bilingual_mode,
            translation_config=translation_config,
        )
    if "ass" in fmt:
        return SubtitleWriter(
            audio_path.with_suffix(".ass"), "ass",
            bilingual_mode=bilingual_mode,
            translation_config=translation_config,
        )
